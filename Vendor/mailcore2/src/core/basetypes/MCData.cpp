...
#ifdef _MSC_VER
    r = _wfopen_s(&f, reinterpret_cast<const wchar_t *>(filename->unicodeCharacters()), L"rb");
    if (r != 0) {
        f = NULL;
    }
#else
...
#ifdef _MSC_VER
    FILE * f = NULL;
    int r = _wfopen_s(&f, reinterpret_cast<const wchar_t *>(filename->unicodeCharacters()), L"wb");
    if (r != 0) {
        f = NULL;
    }
#else
