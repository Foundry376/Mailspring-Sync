...
void _moveMessagesResilient(mailcore::IMAPSession * session, mailcore::String * path, Folder * destFolder, mailcore::IndexSet * uids, std::vector<std::shared_ptr<Message>> messages) {
    mailcore::ErrorCode err = mailcore::ErrorNone;
    mailcore::HashMap * uidmap = nullptr;
    mailcore::String * destPath = AS_MCSTR(destFolder->path());
    bool mustApplyAttributes = false;

    // First, perform the action - either the MOVE or the COPY, STORE, EXPUNGE
    // if mailcore::IMAPCapabilityMove is not present.
    if (session->storedCapabilities()->containsIndex(mailcore::IMAPCapabilityMove)) {
        ...
        session->storeFlagsByUID(path, uids, mailcore::IMAPStoreFlagsRequestKindAdd, mailcore::MessageFlagDeleted, &err);
    ...
    // which UIDs in the old folder std::map to which UIDs in the new folder.
    if (uidmap != nullptr) {
        for (auto msg : messages) {
            mailcore::Value * currentUID = mailcore::Value::valueWithUnsignedLongValue(msg->remoteUID());
            mailcore::Value * newUID = (mailcore::Value *)uidmap->objectForKey(currentUID);
            if (!newUID) {
        ...
        mailcore::IMAPMessagesRequestKind kind = MailUtils::messagesRequestKindFor(session->storedCapabilities(), true);

        if (status != nullptr) {
            uint32_t min = status->uidNext() - (uint32_t)messages.size() * 2;
            if (min < 1) min = 1;
            mailcore::IndexSet * set = mailcore::IndexSet::indexSetWithRange(mailcore::RangeMake(min, UINT64_MAX));
            mailcore::Array * movedMessages = session->fetchMessagesByUID(destPath, kind, set, nullptr, &err);
            for (auto msg : messages) {
                bool found = false;
                for (unsigned int ii = 0; ii < movedMessages->count(); ii ++) {
                    mailcore::IMAPMessage * movedMessage = (mailcore::IMAPMessage*)movedMessages->objectAtIndex(ii);
                    std::string movedId = MailUtils::idForMessage(msg->accountId(), destFolder->path(), movedMessage);
                    if (msg->id() == movedId) {
                        msg->setRemoteFolder(destFolder);
    ...
    if (mustApplyAttributes) {
        for (auto msg : messages) {
            if (msg->remoteFolderId() == destFolder->id()) {
                mailcore::MessageFlag flags = mailcore::MessageFlagNone;
                if (msg->isStarred())
                    flags = (mailcore::MessageFlag)(flags | mailcore::MessageFlagFlagged);
                if (!msg->isUnread())
                    flags = (mailcore::MessageFlag)(flags | mailcore::MessageFlagSeen);
                if (msg->isDraft())
                    flags = (mailcore::MessageFlag)(flags | mailcore::MessageFlagDraft);

                if (flags != mailcore::MessageFlagNone) {
                    session->storeFlagsByUID(destPath, mailcore::IndexSet::indexSetWithIndex(msg->remoteUID()), mailcore::IMAPStoreFlagsRequestKindSet, flags, &err);
                }
            }
        }
    }
...
void _removeMessagesResilient(mailcore::IMAPSession * session, MailStore * store, std::string accountId, mailcore::String * path, mailcore::IndexSet * uids) {
    mailcore::ErrorCode err = mailcore::ErrorNone;
    ...
    mailcore::String * trashPath = nullptr;
    if (session->storedCapabilities()->containsIndex(mailcore::IMAPCapabilityMove)) {
    ...
    if (trashPath != nullptr) {
        mailcore::HashMap * uidMapping = nullptr;
        ...
        if (err != mailcore::ErrorNone) {
            ...
            if (uidMapping) {
                mailcore::Array * uidsInNewFolder = uidMapping->allValues();
                for (unsigned int ii = 0; ii < uidsInNewFolder->count(); ii ++) {
                    mailcore::Value * val = (mailcore::Value *)uidsInNewFolder->objectAtIndex(ii);
...
void _applyUnread(Message * msg, nlohmann::json & data) {
    msg->setUnread(data["unread"].get<bool>());
}

void _applyUnreadInIMAPFolder(mailcore::IMAPSession * session, mailcore::String * path, mailcore::IndexSet * uids, std::vector<std::shared_ptr<Message>> messages, nlohmann::json & data) {
    mailcore::ErrorCode err = mailcore::ErrorNone;
    if (data["unread"].get<bool>() == false) {
        session->storeFlagsByUID(path, uids, mailcore::IMAPStoreFlagsRequestKindAdd, mailcore::MessageFlagSeen, &err);
    } else {
        session->storeFlagsByUID(path, uids, mailcore::IMAPStoreFlagsRequestKindRemove, mailcore::MessageFlagSeen, &err);
    }
    if (err != mailcore::ErrorNone) {
...
void _applyStarred(Message * msg, nlohmann::json & data) {
    msg->setStarred(data["starred"].get<bool>());
}

void _applyStarredInIMAPFolder(mailcore::IMAPSession * session, mailcore::String * path, mailcore::IndexSet * uids, std::vector<std::shared_ptr<Message>> messages, nlohmann::json & data) {
    mailcore::ErrorCode err = mailcore::ErrorNone;
    if (data["starred"].get<bool>() == true) {
        session->storeFlagsByUID(path, uids, mailcore::IMAPStoreFlagsRequestKindAdd, mailcore::MessageFlagFlagged, &err);
    } else {
        session->storeFlagsByUID(path, uids, mailcore::IMAPStoreFlagsRequestKindRemove, mailcore::MessageFlagFlagged, &err);
    }
    if (err != mailcore::ErrorNone) {
        throw SyncException(err, "storeFlagsByUID");
    }
...
void _applyFolder(Message * msg, nlohmann::json & data) {
    Folder folder{data["folder"]};
    msg->setClientFolder(&folder);
}

void _applyFolderMoveInIMAPFolder(mailcore::IMAPSession * session, mailcore::String * path, mailcore::IndexSet * uids, std::vector<std::shared_ptr<Message>> messages, nlohmann::json & data) {
    Folder destFolder{data["folder"]};

    _moveMessagesResilient(session, path, &destFolder, uids, messages);
}

std::string _xgmKeyForLabel(nlohmann::json & label) {
    std::string role = label["role"].get<std::string>();
    std::string path = label["path"].get<std::string>();
    if (role == "inbox") {
        return "\\Inbox";
    }
    if (role == "important") {
...
void _applyLabels(Message * msg, nlohmann::json & data) {
    nlohmann::json & toAdd = data["labelsToAdd"];
    nlohmann::json & toRemove = data["labelsToRemove"];
    nlohmann::json & labels = msg->remoteXGMLabels();

    for (auto & item : toAdd) {
        std::string xgmValue = _xgmKeyForLabel(item);
        bool found = false;
        for (auto & existing : labels) {
            if (existing.get<std::string>() == xgmValue) {
                found = true;
                break;
            }
        }
    ...
    for (auto & item : toRemove) {
        ...
        for (int i = (int)labels.size() - 1; i >= 0; i --) {
            if (labels.at(i).get<std::string>() == xgmValue) {
                labels.erase(i);
            }
        }
    }
...
void _applyLabelChangeInIMAPFolder(mailcore::IMAPSession * session, mailcore::String * path, mailcore::IndexSet * uids, std::vector<std::shared_ptr<Message>> messages, nlohmann::json & data) {
    mailcore::AutoreleasePool pool;

    mailcore::ErrorCode err = mailcore::ErrorNone;
    mailcore::Array * toAdd = new mailcore::Array{};
    toAdd->autorelease();
    for (auto & item : data["labelsToAdd"]) {
        toAdd->addObject(AS_MCSTR(_xgmKeyForLabel(item)));
    }

    mailcore::Array * toRemove = new mailcore::Array{};
    ...
    if (toAdd->count() > 0) {
        session->storeLabelsByUID(path, uids, mailcore::IMAPStoreFlagsRequestKindAdd, toAdd, &err);
        if (err != mailcore::ErrorNone) {
            throw SyncException(err, "storeLabelsByUID - add");
        }
    }
    if (toRemove->count() > 0) {
        session->storeLabelsByUID(path, uids, mailcore::IMAPStoreFlagsRequestKindRemove, toRemove, &err);
        if (err != mailcore::ErrorNone) {
...
TaskProcessor::TaskProcessor(std::shared_ptr<Account> account, MailStore * store, mailcore::IMAPSession * session) :
    account(account),
    store(store),
...
void TaskProcessor::cleanupOldTasksAtRuntime() {
    ...
    if (countToRemove > 10) { // slop
        ...
        std::vector<std::shared_ptr<Task>> unneededTasks{};
        while (unneeded.executeStep()) {
            unneededTasks.push_back(std::make_shared<Task>(unneeded));
        }
        unneeded.reset();
...
void TaskProcessor::performLocal(Task * task) {
    std::string cname = task->constructorName();
...
void TaskProcessor::cancel(std::string taskId) {
...
Message TaskProcessor::inflateClientDraftJSON(nlohmann::json & draftJSON, std::shared_ptr<Message> existing = nullptr) {
...
ChangeMailModels TaskProcessor::inflateMessages(nlohmann::json & data) {
    ChangeMailModels models;

    if (data.count("threadIds")) {
        std::vector<std::string> threadIds{};
        for (auto & member : data["threadIds"]) {
            threadIds.push_back(member.get<std::string>());
        }
        ...
        for (auto & member : data["messageIds"]) {
            messageIds.push_back(member.get<std::string>());
        }
...
void TaskProcessor::performLocalChangeOnMessages(Task * task, void (*modifyLocalMessage)(Message *, nlohmann::json &)) {
    MailStoreTransaction transaction{store, "performLocalChangeOnMessages"};

    nlohmann::json & data = task->data();
...
void TaskProcessor::performRemoteChangeOnMessages(Task * task, bool updatesFolder, void (*applyInFolder)(mailcore::IMAPSession * session, mailcore::String * path, mailcore::IndexSet * uids, std::vector<std::shared_ptr<Message>> messages, nlohmann::json & data)) {
    ...
    std::vector<std::shared_ptr<Message>> messages = inflateMessages(data).messages;
    std::map<std::string, std::shared_ptr<mailcore::IndexSet>> uidsByFolder{};
    std::map<std::string, std::vector<std::shared_ptr<Message>>> msgsByFolder{};
    std::map<std::string, std::shared_ptr<Message>> messagesById{};

    for (auto msg : messages) {
        std::string path = msg->remoteFolder()["path"].get<std::string>();
        uint32_t uid = msg->remoteUID();
        if (!uidsByFolder.count(path)) {
            uidsByFolder[path] = std::make_shared<mailcore::IndexSet>();
            msgsByFolder[path] = {};
        }
    ...
    for (auto pair : msgsByFolder) {
        ...
        mailcore::IndexSet * uids = uidsByFolder[pair.first].get();
    ...
    // by applyInFolder and decrement locks.
    {
        ...
        std::vector<std::shared_ptr<Message>> safeMessages = inflateMessages(data).messages;
...
void TaskProcessor::performLocalSaveDraft(Task * task) {
    nlohmann::json & draftJSON = task->data()["draft"];

    {
        ...
        std::shared_ptr<Message> existing = nullptr;
        if (draftJSON.count("id")) {
            existing = store->find<Message>(Query().equal("id", draftJSON["id"].get<std::string>()));
        }
        ...
        if (draftJSON.count("body")) {
            ...
            insert.bind(2, draftJSON["body"].get<std::string>());
            insert.exec();
        }
        transaction.commit();
    }
...
void TaskProcessor::performLocalDestroyDraft(Task * task) {
    std::vector<std::string> messageIds = task->data()["messageIds"];
    ...
    auto stubIds = nlohmann::json::array();
...
void TaskProcessor::performRemoteDestroyDraft(Task * task) {
    std::vector<std::string> stubIds = task->data()["stubIds"];
    ...
    for (auto & stub : stubs) {
        ...
        auto uids = mailcore::IndexSet::indexSetWithIndex(stub->remoteUID());
        mailcore::String * path = AS_MCSTR(stub->remoteFolder()["path"].get<std::string>());
...
void TaskProcessor::performLocalDestroyContact(Task * task) {
    ...
    for (nlohmann::json & c : task->data()["contacts"]) {
        contactIds.push_back(c["id"].get<std::string>());
    }
...
void TaskProcessor::performRemoteDestroyContact(Task * task) {
    ...
    if (account->provider() == "gmail") {
        auto deleted = store->findLargeSet<Contact>("id", contactIds);
        auto gpeople = std::make_shared<GoogleContactsWorker>(account);
        ...
        auto deleted = store->findLargeSet<Contact>("id", contactIds);
        auto dav = std::make_shared<DAVWorker>(account);
        for (auto & contact : deleted) {
...
void TaskProcessor::performLocalSyncbackContactGroup(Task * task) {
    std::string id = task->data()["group"].count("id") ? task->data()["group"]["id"].get<std::string>() : "";
    std::string name = task->data()["group"]["name"].get<std::string>();
    ...
    if (account->provider() == "gmail") {
        ...
        if (!local) {
            local = std::make_shared<ContactGroup>(id, account->id());
        }
        ...
        if (id != "") {
            ...
            contact->mutateCardInInfo([&](std::shared_ptr<VCard> card) {
                if (card->getName()) {
            ...
            auto contact = std::make_shared<Contact>(uid, account->id(), "", CONTACT_MAX_REFS, CARDDAV_SYNC_SOURCE);
            contact->setInfo(nlohmann::json::object({{"vcf", "BEGIN:VCARD\r\nVERSION:3.0\r\nUID:"+uid+"\r\nEND:VCARD\r\n"}, {"href", ""}}));
            contact->setHidden(true);
            contact->setName(name);
            contact->mutateCardInInfo([&](std::shared_ptr<VCard> vcard) {
                vcard->setName(name);
                vcard->addProperty(std::make_shared<VCardProperty>("FN", name));
                vcard->addProperty(std::make_shared<VCardProperty>(X_VCARD3_KIND, "group"));
            });
...
void TaskProcessor::performLocalDestroyContactGroup(Task * task) {
    std::string id = task->data()["group"]["id"].get<std::string>();
...
void TaskProcessor::performRemoteDestroyContactGroup(Task * task) {
    ...
    if (account->provider() == "gmail") {
        ...
        auto resourceName = task->data()["googleResourceName"].get<std::string>();
        auto gpeople = std::make_shared<GoogleContactsWorker>(account);
...
void TaskProcessor::performLocalChangeContactGroupMembership(Task * task) {
    ...
    auto direction = task->data()["direction"].get<std::string>();
    auto groupId = task->data()["group"]["id"].get<std::string>();
    ...
    if (account->provider() == "gmail") {
        ...
        contactForGroup->mutateCardInInfo([&](std::shared_ptr<VCard> card) {
            if (direction == "add") {
...
void TaskProcessor::performLocalSyncbackContact(Task * task) {
    auto clientside = std::make_shared<Contact>(task->data()["contact"]);
    ...
    auto local = task->data()["contact"].count("id")
        ...
        : std::make_shared<Contact>(MailUtils::idRandomlyGenerated(), account->id(), "", CONTACT_MAX_REFS, source);
...
void TaskProcessor::performRemoteSyncbackContact(Task * task) {
    std::string id = task->data()["contact"]["id"].get<std::string>();
...
void TaskProcessor::performRemoteSyncbackCategory(Task * task) {
    nlohmann::json & data = task->data();
    std::string accountId = task->accountId();
    std::string path = data["path"].get<std::string>();
    std::string existingPath = data.count("existingPath") ? data["existingPath"].get<std::string>() : "";
    ...
    std::shared_ptr<Folder> localModel = nullptr;
    ...
    if (!localModel) {
        if (isGmail) {
            localModel = std::make_shared<Label>(MailUtils::idForFolder(accountId, path), accountId, 0);
        } else {
            localModel = std::make_shared<Folder>(MailUtils::idForFolder(accountId, path), accountId, 0);
        }
    }
...
void TaskProcessor::performLocalSyncbackMetadata(Task * task) {
    nlohmann::json & data = task->data();
    std::string aid = task->accountId();
    std::string id = data["modelId"];
    std::string type = data["modelClassName"];
    std::string pluginId = data["pluginId"];
    nlohmann::json & value = data["value"];
...
void TaskProcessor::performRemoteSyncbackMetadata(Task * task) {
    ...
    if (data["modelHeaderMessageId"].is_string()) {
        payload["headerMessageId"] = data["modelHeaderMessageId"].get<std::string>();
    }

    const nlohmann::json results = PerformIdentityRequest("/metadata/" + account->id() + "/" + id + "/" + pluginId, "POST", payload);
...
void TaskProcessor::performRemoteSendDraft(Task * task) {
    ...
    nlohmann::json & draftJSON = task->data()["draft"];
    nlohmann::json & perRecipientBodies = task->data()["perRecipientBodies"];
    std::string body = draftJSON["body"].get<std::string>();
    ...
    mailcore::String * sentPath = AS_MCSTR(sent->path());
    logger->info("-- Identified `sent` folder: {}", sent->path());

    // build the MIME message
    mailcore::MessageBuilder builder;
    if (multisend) {
        ...
        if (plaintext) {
            builder.setTextBody(AS_MCSTR(perRecipientBodies["self"].get<std::string>()));
        } else {
            builder.setHTMLBody(AS_MCSTR(perRecipientBodies["self"].get<std::string>()));
        }
    } else {
        if (plaintext) {
    ...
    if (draft.replyToHeaderMessageId() != "") {
        builder.header()->setReferences(mailcore::Array::arrayWithObject(AS_MCSTR(draft.replyToHeaderMessageId())));
        builder.header()->setInReplyTo(mailcore::Array::arrayWithObject(AS_MCSTR(draft.replyToHeaderMessageId())));
    }
    if (draft.forwardedHeaderMessageId() != "") {
        builder.header()->setReferences(mailcore::Array::arrayWithObject(AS_MCSTR(draft.forwardedHeaderMessageId())));
    }

    mailcore::Array * to = new mailcore::Array();
    for (nlohmann::json & p : draft.to()) {
        to->addObject(MailUtils::addressFromContactJSON(p));
    }
    builder.header()->setTo(to);

    mailcore::Array * cc = new mailcore::Array();
    for (nlohmann::json & p : draft.cc()) {
        cc->addObject(MailUtils::addressFromContactJSON(p));
    }
    builder.header()->setCc(cc);

    mailcore::Array * bcc = new mailcore::Array();
    for (nlohmann::json & p : draft.bcc()) {
        bcc->addObject(MailUtils::addressFromContactJSON(p));
    }
    builder.header()->setBcc(bcc);

    mailcore::Array * replyTo = new mailcore::Array();
    for (nlohmann::json & p : draft.replyTo()) {
        replyTo->addObject(MailUtils::addressFromContactJSON(p));
    }
    builder.header()->setReplyTo(replyTo);

    nlohmann::json & fromP = draft.from().at(0);
    builder.header()->setFrom(MailUtils::addressFromContactJSON(fromP));

    for (nlohmann::json & fileJSON : draft.files()) {
        File file{fileJSON};
        std::string root = MailUtils::getEnvUTF8("CONFIG_DIR_PATH") + FS_PATH_SEP + "files";
        std::string path = MailUtils::pathForFile(root, &file, false);

#ifdef _MSC_VER
        wstring_convert<std::codecvt_utf8<wchar_t>, wchar_t> convert;
        mailcore::Attachment * a = mailcore::Attachment::attachmentWithContentsOfFile(AS_WIDE_MCSTR(convert.from_bytes(path)));
#else
        mailcore::Attachment * a = mailcore::Attachment::attachmentWithContentsOfFile(AS_MCSTR(path));
#endif

        if (file.contentId().is_string()) {
            a->setContentID(AS_MCSTR(file.contentId().get<std::string>()));
    ...
    mailcore::Data * messageDataForSent = builder.data();
    ...
    */
    ...
    if (multisend) {
        ...
        for (nlohmann::json::iterator it = perRecipientBodies.begin(); it != perRecipientBodies.end(); ++it) {
            if (it.key() == "self") {
                continue;
            }
            ...
            if (plaintext) {
                builder.setTextBody(AS_MCSTR(it.value().get<std::string>()));
            } else {
                builder.setHTMLBody(AS_MCSTR(it.value().get<std::string>()));
            }
            mailcore::Address * to = mailcore::Address::addressWithMailbox(AS_MCSTR(it.key()));
            mailcore::Data * messageData = builder.data();
            smtp.sendMessage(builder.header()->from(), mailcore::Array::arrayWithObject(to), messageData, &sprogress, &err);
            if (err != mailcore::ErrorNone) {
                break;
            }
            succeeded += "\n - " + it.key();
    ...
    if (err != mailcore::ErrorNone) {
        ...
        std::string es = LibEtPanCodeToTypeMap.count(e) ? LibEtPanCodeToTypeMap[e] : std::to_string(e);
    ...
    if (draft.remoteUID() != 0) {
        auto uids = mailcore::IndexSet::indexSetWithIndex(draft.remoteUID());
        mailcore::String * path = AS_MCSTR(draft.remoteFolder()["path"].get<std::string>());
    ...
    if (sentFolderMessageUID == 0) {
        ...
        session->appendMessage(sentPath, messageDataForSent, mailcore::MessageFlagSeen, &iprogress, &sentFolderMessageUID, &err);
        if (err != mailcore::ErrorNone) {
        ...
        // the sent folder.
        if (session->storedCapabilities()->containsIndex(mailcore::IMAPCapabilityGmail)) {
            if (draft.threadId() != "") {
                auto thread = store->find<Thread>(Query().equal("id", draft.threadId()));
                if (thread) {
                    mailcore::Array * xgmValues = new mailcore::Array();
                    for (auto & l : thread->labels()) {
                        std::string role = l["role"].get<std::string>();
                        if (role == "inbox" || role == "sent" || role == "drafts") { continue; }
                        std::string xgm = _xgmKeyForLabel(l);
                        logger->info("-- Will add label to new message: {}", xgm);
                        xgmValues->addObject(AS_MCSTR(xgm));
                    }
                    session->storeLabelsByUID(sentPath, mailcore::IndexSet::indexSetWithIndex(sentFolderMessageUID), mailcore::IMAPStoreFlagsRequestKindAdd, xgmValues, &err);
                    if (err != mailcore::ErrorNone) {
    ...
    std::shared_ptr<Message> localMessage = nullptr;
    mailcore::IMAPMessage * remoteMessage = nullptr;

    logger->info("-- Syncing sent message (UID {}) to the local mail store", sentFolderMessageUID);
    mailcore::IMAPMessagesRequestKind kind = (mailcore::IMAPMessagesRequestKind)(mailcore::IMAPMessagesRequestKindHeaders | mailcore::IMAPMessagesRequestKindFlags);
    if (session->storedCapabilities()->containsIndex(mailcore::IMAPCapabilityGmail)) {
        kind = (mailcore::IMAPMessagesRequestKind)(kind | mailcore::IMAPMessagesRequestKindGmailLabels | mailcore::IMAPMessagesRequestKindGmailThreadID | mailcore::IMAPMessagesRequestKindGmailMessageID);
    }
    ...
    mailcore::IndexSet * uids = mailcore::IndexSet::indexSetWithIndex(sentFolderMessageUID);
    mailcore::Array * remote = session->fetchMessagesByUID(sentPath, kind, uids, nullptr, &err);
    ...
    mailcore::MessageParser * messageParser = mailcore::MessageParser::messageParserWithData(messageDataForSent);
    remoteMessage = (mailcore::IMAPMessage *)(remote->lastObject());
    ...
    for (const auto & m : draft.metadata()) {
        auto pluginId = m["pluginId"].get<std::string>();
...
void TaskProcessor::performRemoteSendFeatureUsageEvent(Task * task) {
    const auto feature = task->data()["feature"].get<std::string>();
    nlohmann::json payload = {
        {"feature", feature}
    };
...
void TaskProcessor::performLocalChangeRoleMapping(Task * task) {
    const auto path = task->data()["path"].get<std::string>();
    const auto role = task->data()["role"].get<std::string>();

    {
        ...
        std::shared_ptr<Folder> category = store->find<Folder>(query);
        if (category == nullptr) {
        ...
        std::shared_ptr<Folder> existing = store->find<Folder>(existingQuery);
        if (existing == nullptr) {
...
void TaskProcessor::performRemoteExpungeAllInFolder(Task * task) {
    mailcore::AutoreleasePool pool;
    mailcore::ErrorCode err = mailcore::ErrorNone;
    const auto path = task->data()["folder"]["path"].get<std::string>();
    const auto id = task->data()["folder"]["id"].get<std::string>();

    mailcore::IndexSet set;
    set.addRange(mailcore::RangeMake(1, UINT64_MAX));
    session->storeFlagsByUID(AS_MCSTR(path), &set, mailcore::IMAPStoreFlagsRequestKindAdd, mailcore::MessageFlagDeleted, &err);
    if (err != mailcore::ErrorNone) {
        throw SyncException(err, "storeFlagsByUID");
    }
...
void TaskProcessor::performRemoteGetMessageRFC2822(Task * task) {
    ...
    IMAPProgress cb;
    mailcore::ErrorCode err = mailcore::ErrorNone;
    const auto id = task->data()["messageId"].get<std::string>();
    const auto filepath = task->data()["filepath"].get<std::string>();

    auto msg = store->find<Message>(Query().equal("id", id));

    mailcore::Data * data = session->fetchMessageByUID(AS_MCSTR(msg->remoteFolder()["path"].get<std::string>()), msg->remoteUID(), &cb, &err);
    if (err != mailcore::ErrorNone) {
...
void TaskProcessor::performRemoteSendRSVP(Task * task) {
    ...
    // load the draft and body from the task
    std::string ics = task->data()["ics"].get<std::string>();
    std::string icsMethod = "REPLY";
    std::string subject = task->data()["subject"].get<std::string>();
    std::string organizer = task->data()["to"].get<std::string>();
    ...
    mailcore::Array * to = new mailcore::Array();
    to->addObject(mailcore::Address::addressWithMailbox(AS_MCSTR(organizer)));
    builder.header()->setTo(to);

    mailcore::Array * replyTo = new mailcore::Array();
    mailcore::Address * me = mailcore::Address::addressWithMailbox(AS_MCSTR(account->emailAddress()));
    ...
    mailcore::Data * data;
    mailcore::Attachment * icsAttachment;

    icsAttachment = new mailcore::Attachment();
