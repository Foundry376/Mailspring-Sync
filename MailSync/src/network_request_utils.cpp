...
std::string FindLinuxCertsBundle() {
#ifdef __linux__
...
const nlohmann::json MakeOAuthRefreshRequest(std::string provider, std::string clientId, std::string refreshToken) {
    ...
    std::string payload = "grant_type=refresh_token&client_id=" + std::string(c) + "&refresh_token=" + std::string(r);
...
CURL * CreateJSONRequest(std::string url, std::string method, std::string authorization, const char * payloadChars) {
    CURL * curl_handle = curl_easy_init();
    ...
    std::string explicitCertsBundlePath = FindLinuxCertsBundle();
...
const nlohmann::json PerformJSONRequest(CURL * curl_handle) {
    ...
    nlohmann::json resultJSON = nullptr;
    try {
...
void ValidateRequestResp(CURLcode res, CURL * curl_handle, std::string resp) {
    ...
    if (http_code < 200 || http_code > 209) {
        ...
        if (resp.find("invalid_grant") != std::string::npos) {
            retryable = false;
        }
...
CURL * CreateIdentityRequest(std::string path, std::string method, const char * payloadChars) {
    std::string url { MailUtils::getEnvUTF8("IDENTITY_SERVER") + path };
    std::string plain = Identity::GetGlobal()->token() + ":";
    std::string encoded = MailUtils::toBase64(plain.c_str(), strlen(plain.c_str()));
    std::string authorization = "Basic " + encoded;
    return CreateJSONRequest(url, method, authorization, payloadChars);
}

const nlohmann::json PerformIdentityRequest(std::string path, std::string method, const nlohmann::json & payload) {
